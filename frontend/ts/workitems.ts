...
export function toggleExpand(id: string): void {
  ...
  // Flip the arrow
  const arrow = el.previousElementSibling?.querySelector('.expand-arrow') as HTMLElement | null;
...
export function openWorkItemModal(): void {
  ...
  (document.getElementById('workItemRepo') as HTMLInputElement).value    = '';
...
export async function saveWorkItem(): Promise<void> {
  ...
  const repo = (document.getElementById('workItemRepo') as HTMLInputElement).value.trim();
  if (!desc) return toast('Description is required');
  try {
    await api<WorkItem>('/work-items', 'POST', { description: desc, repo });
    await loadWorkItems();
    renderWorkItems();
